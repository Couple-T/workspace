...
3. **Important findings → share early.** As soon as you confirm something important, add a comment on the MR/PR at the specific **file:line / scope** and **`SendMessage` Noah a one-line pointer** — then **keep reviewing**; no need to wait (Daniel and Liam review in parallel). Noah queues it into his single FIFO and pings you per fix; re-review just that scope when he does.
