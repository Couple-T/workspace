...
4. **Publish onto the ticket — `/update-ticket <FM>`.** Confirm/keep **`Status → Testing`** while the results land — **you never set `Status → Done`.** Done is qa-planner's final verdict (§6); on a red suite it obviously stays `Testing` too. `report-test-results` only comments; this step is the Status move — to `Testing`, never `Done`.
